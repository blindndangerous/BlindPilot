...
it. That was fixed once, when the strip and the pages were the same control;
splitting them brought it back, because wxSimplebook focuses the page it shows
from C++ on every selection change, whether or not anybody asked it to.

And Tab out of the strip was routed by hand, which meant a route that did not
...
# Held for the process: a wx.App that nothing refers to is collected the
# moment it is made, and everything after it fails asking for one.
_APP: list[wx.App] = []


def _app() -> wx.App:
    if wx.GetApp() is None:
        _APP.append(wx.App(False))
    return wx.GetApp()
...
def test_arrowing_the_strip_leaves_focus_on_the_strip(monkeypatch, tmp_path):
    ...
    try:
        ...
        # Where focus actually is cannot be read back on a headless runner, so
        # say it is on the strip - which is the case this is about.
        monkeypatch.setattr(
        ...
        # The page cannot accept the focus the book offers it, and the strip is
        # asked for it back either way.
        assert enabled_during_switch == [False]
        assert incoming.IsEnabled()
        assert strip_focus == ["strip"]
        # The native tab control says which tab this is; saying it again here
        # would speak everything twice.
        assert spoken == []
    finally:
        frame.Destroy()

...
FreeBuff ships no JSON or headless API, so BlindPilot runs its terminal interface in a hidden pseudo-terminal, reads the answer off its screen a sentence at a time, and captures its chat id to resume. Redraws and advertisements are filtered before anything is spoken. Its permission picker and Compact Conversation are disabled because the CLI has no equivalent. FreeBuff renames and drops models between releases, so the default is a preference: GLM 5.3 Flash when it is on offer, FreeBuff's own choice when it is not. Because a message typed into FreeBuff's composer before its runtime has reconnected is lost silently, BlindPilot holds the message until the boot's reconnection is visible in FreeBuff's own chat log — "FreeBuff is still starting; holding the message until it is ready" — and sends it the moment the session is live. When a session genuinely drops mid-turn, BlindPilot watches briefly for the automatic rejoin before saying so and naming the remedy instead of waiting out the turn.
